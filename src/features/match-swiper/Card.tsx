  ...
  return (
    <Pressable style={styles.cardPressable} onPress={onPress}>
      <Animated.View style={[styles.card, { transform: [{ scale: scaleAnim }] }]}> 
        ...
        />
        {rest.backgroundLogo && (
          <Image source={{ uri: rest.backgroundLogo }} style={styles.leagueBgLogo} resizeMode="contain" />
        )}
        <View style={styles.leagueBlock}>
          {rest.league && (
            <View style={styles.leagueRow}>
              <Text style={styles.leagueIcon}>🏆</Text>
              <Typography variant="body" weight="bold" font="Oswald" color="#fff" style={styles.leagueText}>
                {rest.league.split(/\n|\r\n/)[0]}
              </Typography>
            </View>
          )}
          {rest.time && (
            <Typography variant="caption" weight="regular" font="Oswald" color="#fff" style={styles.weekText}>
              {rest.time.split(/\n|\r\n/)[0]}
            </Typography>
          )}
        </View>
        {rest.isLive && (
          <View style={styles.liveBlock}>
            <Typography variant="caption" weight="bold" font="Oswald" color="#fff" style={styles.liveText}>
              LIVE
            </Typography>
        ...
        <View style={[styles.scoreRow, { position: 'absolute', bottom: 80, left: 0, right: 0 }]}> 
          ...
          <View style={styles.vsCircle}>
            <Typography variant="body" weight="bold" font="Oswald" color="#fff" style={styles.vsText}>
              vs
            </Typography>
          </View>
          <Typography variant="h1" weight="bold" font="Oswald" color="#fff" style={styles.score}>
        ...
        <View style={[styles.teamsRow, { position: 'absolute', bottom: 10, left: 20, width: '35%', justifyContent: 'flex-end', alignItems: 'center', flexDirection: 'row' }]}> 
          <Typography variant="caption" weight="bold" font="Oswald" color="#fff" style={[styles.teamName, { textAlign: 'right', marginLeft: 8 }]} numberOfLines={1}>
            {rest.homeTeam?.name}
          </Typography>
        </View>
        <View style={[styles.teamsRow, { position: 'absolute', bottom: 20, right: 20, width: '35%', flexDirection: 'row', alignItems: 'center' }]}> 
          <View style={{ position: 'absolute', zIndex: 100 }}>
            <Typography variant="caption" weight="bold" font="Oswald" color="#fff" style={[styles.teamName, { textAlign: 'left', marginRight: 8 }]} numberOfLines={1}>
              {rest.awayTeam?.name}
            </Typography>
          </View>
        </View>
        <View style={[styles.bottomBlock, { position: 'absolute', bottom: 16, left: 0, right: 0 }]}> 
          {rest.badgeText && (
            <View style={styles.badge}>
              <Typography variant="body" weight="bold" font="Oswald" color="#1ED760" style={styles.badgeText}>
                {rest.badgeText}
              </Typography>
            </View>
          )}
          {rest.stadium && (
            <Typography variant="caption" weight="regular" font="Oswald" color="#888" style={styles.stadium}>
              {rest.stadium}
            </Typography>
          )}
        </View>
