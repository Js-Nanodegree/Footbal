import { StandingsResponseSchema } from '../zod/standingsResponseSchema';
import { tmpApiAxios } from '../clients/tmpApiAxios';
import { Standing } from '../types/standing';

const BASE_URL = 'https://api.football-data.org/v4';

export async function getStandingsEndpoint(
  competitionId: number,
  client: 'fetch' | 'axios' = 'fetch',
  API_KEY: string,
  axiosOptions?: any
): Promise<Standing[]> {
  const url = `/competitions/${competitionId}/standings`;
  if (client === 'axios') {
    const data = await tmpApiAxios.request({
      url,
      method: 'get',
      ...(axiosOptions || {}),
    });
    const parsed = StandingsResponseSchema.parse(data);
    return parsed.standings as Standing[];
  } else {
    const res = await fetch(BASE_URL + url, { headers: { 'X-Auth-Token': API_KEY } });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    const parsed = StandingsResponseSchema.parse(data);
    return parsed.standings as Standing[];
  }
} 