    ...
    <View style={[styles.card, { backgroundColor: matchEventColorMap.background }]}>
      <View style={styles.header}>
        ...
        {event.score && (
          <View style={styles.scoreBlock}>
            {event.teamLogoLeft && (
              <Image source={{ uri: event.teamLogoLeft }} style={styles.logo} />
            )}
            <Typography variant="body" weight="bold" style={{ marginHorizontal: 4 }}>
              {event.score}
            </Typography>
            {event.teamLogoRight && (
              <Image source={{ uri: event.teamLogoRight }} style={styles.logo} />
            )}
          </View>
        )}
      </View>
      ...
      {event.assist && (
        <Typography variant="caption" color={matchEventColorMap.assist} style={{ marginTop: 2 }}>
          Goal Assist:{' '}
          <Typography variant="caption" weight="bold" color={matchEventColorMap.assist}>
            {event.assist.name}
          </Typography>
        </Typography>
      )}
        ...
        <View style={{ marginTop: 4 }}>
          ...
          <View style={styles.playerRow}>
            ...
            <View style={{ flex: 1 }}>
              <Typography
                variant="caption"
                color={matchEventColorMap.out}
                style={{ fontWeight: 'bold' }}
              >
                OUT
              </Typography>
              <Typography variant="body" weight="bold">
                {event.outPlayer.name}
              </Typography>
              <Typography variant="caption" color={matchEventColorMap.default}>
